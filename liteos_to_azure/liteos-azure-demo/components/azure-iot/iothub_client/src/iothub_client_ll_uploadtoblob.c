...
{
    IOTHUB_CLIENT_LL_UPLOADTOBLOB_HANDLE_DATA* handleData = platform_malloc(sizeof(IOTHUB_CLIENT_LL_UPLOADTOBLOB_HANDLE_DATA));
    if (handleData == NULL)
    {
        LogError("oom - malloc");
        /*return as is*/
    ...
    else
    {
        ...
        {
            handleData->hostname = platform_malloc(iotHubNameLength + 1 + iotHubSuffixLength + 1); /*first +1 is because "." the second +1 is because \0*/
            ...
            else
            {
                ...
                *insert_pos = '\0';
                ...
                if ((config->deviceSasToken != NULL) && (config->deviceKey == NULL))
                {
                    ...
                    {
                        LogError("unable to STRING_construct");
                        platform_free((void*)handleData->hostname);
                        STRING_delete(handleData->deviceId);
                        platform_free(handleData);
                        handleData = NULL;
                    }
                                    ...
                                    else
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            else
                                            {
                                                ...
                                                char * requiredString = platform_malloc(requiredStringLength + 1);
                                                if (requiredString == 0)
                                                {
    ...
    else
    {
        ...
        switch (handleData->authorizationScheme)
        {
            ...
            case(X509):
            {
                ...
                {
                    platform_free((void*)handleData->credentials.x509credentials.x509certificate);
                }
                if (handleData->credentials.x509credentials.x509privatekey != NULL)
                {
                    platform_free((void*)handleData->credentials.x509credentials.x509privatekey);
                }
                break;
            }
            default:
        ...
        {
            platform_free(handleData->certificates);
        }
        if (handleData->http_proxy_options.host_address != NULL)
        {
            platform_free((char *)handleData->http_proxy_options.host_address);
        }
        if (handleData->http_proxy_options.username != NULL)
        {
            platform_free((char *)handleData->http_proxy_options.username);
        }
        if (handleData->http_proxy_options.password != NULL)
        {
            platform_free((char *)handleData->http_proxy_options.password);
        }
        platform_free(handleData);
    }
}
