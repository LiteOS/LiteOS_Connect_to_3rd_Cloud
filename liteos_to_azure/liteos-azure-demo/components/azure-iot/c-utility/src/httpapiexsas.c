...
static HTTPAPIEX_SAS_STATE* construct_httpex_sas(const char* key, const char* uriResource, const char* keyName)
{
    ...
    result = (HTTPAPIEX_SAS_STATE*)platform_malloc(sizeof(HTTPAPIEX_SAS_STATE));
    if (result == NULL)
    {
...
void HTTPAPIEX_SAS_Destroy(HTTPAPIEX_SAS_HANDLE handle)
{
    ...
    if (handle)
    {
        ...
        if (state->key)
        {
            platform_free(state->key);
        }
        if (state->uriResource)
        {
            platform_free(state->uriResource);
        }
        if (state->keyName)
        {
            platform_free(state->keyName);
        }
        platform_free(state);
    }
}
