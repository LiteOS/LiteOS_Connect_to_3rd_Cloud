...
{
    STRING* result;
    if ((result = (STRING*)platform_malloc(sizeof(STRING))) != NULL)
    {
        if ((result->s = (char*)platform_malloc(1)) != NULL)
        {
            result->s[0] = '\0';
        }
        else
...
STRING_HANDLE STRING_clone(STRING_HANDLE handle)
{
    ...
    else
    {
        ...
        if ((result = (STRING*)platform_malloc(sizeof(STRING))) != NULL)
        {
            ...
            if ((result->s = (char*)platform_malloc(sourceLen + 1)) == NULL)
            {
...
STRING_HANDLE STRING_construct(const char* psz)
{
    ...
    {
        STRING* str;
        if ((str = (STRING*)platform_malloc(sizeof(STRING))) != NULL)
        {
            size_t nLen = strlen(psz) + 1;
            if ((str->s = (char*)platform_malloc(nLen)) != NULL)
            {
...
#define STRINGS_C_SPRINTF_BUFFER_SIZE (128)
#if defined(__GNUC__)
...
STRING_HANDLE STRING_construct_sprintf(const char* format, ...)
{
    ...
    if (format != NULL)
    {
        ...
        if (length > 0)
        {
            result = (STRING*)platform_malloc(sizeof(STRING));
            if (result != NULL)
            {
                result->s = (char*)platform_malloc(length+1);
                if (result->s != NULL)
                {
                    va_start(arg_list, format);
...
STRING_HANDLE STRING_new_quoted(const char* source)
{
    ...
    else if ((result = (STRING*)platform_malloc(sizeof(STRING))) != NULL)
    {
        size_t sourceLength = strlen(source);
        if ((result->s = (char*)platform_malloc(sourceLength + 3)) != NULL)
        {
            result->s[0] = '"';
...
STRING_HANDLE STRING_new_JSON(const char* source)
{
    ...
    else
    {
        ...
        else
        {
            ...
            else if ((result->s = (char*)platform_malloc(vlen + 5 * nControlCharacters + nEscapeCharacters + 3)) == NULL)
            {
                /*Codes_SRS_STRING_02_021: [If the complete JSON representation cannot be produced, then STRING_new_JSON shall fail and return NULL.] */
                platform_free(result);
                result = NULL;
...
int STRING_concat(STRING_HANDLE handle, const char* s2)
{
    ...
    else
    {
        ...
        char* temp = (char*)platform_realloc(s1->s, s1Length + s2Length + 1);
        if (temp == NULL)
        {
            /* Codes_SRS_STRING_07_013: [STRING_concat shall return a nonzero number if an error is encountered.] */
...
int STRING_concat_with_STRING(STRING_HANDLE s1, STRING_HANDLE s2)
{
    ...
    else
    {
        ...
        char* temp = (char*)platform_realloc(dest->s, s1Length + s2Length + 1);
        if (temp == NULL)
        {
            /* Codes_SRS_STRING_07_035: [String_Concat_with_STRING shall return a nonzero number if an error is encountered.] */
            LogError("Failure reallocating value");
...
int STRING_copy(STRING_HANDLE handle, const char* s2)
{
    ...
    else
    {
        ...
        {
            size_t s2Length = strlen(s2);
            char* temp = (char*)platform_realloc(s1->s, s2Length + 1);
            if (temp == NULL)
            {
                LogError("Failure reallocating value.");
...
int STRING_copy_n(STRING_HANDLE handle, const char* s2, size_t n)
{
    ...
    else
    {
        ...
        temp = (char*)platform_realloc(s1->s, s2Length + 1);
        if (temp == NULL)
        {
            LogError("Failure reallocating value.");
...
int STRING_sprintf(STRING_HANDLE handle, const char* format, ...)
{
    ...
    else
    {
        ...
        else
        {
            ...
            char* temp;
            size_t s1Length = strlen(s1->s);
            temp = (char*)platform_realloc(s1->s, s1Length + s2Length + 1);
            if (temp != NULL)
            {
                s1->s = temp;
                va_start(arg_list, format);
...
int STRING_quote(STRING_HANDLE handle)
{
    ...
    else
    {
        STRING* s1 = (STRING*)handle;
        size_t s1Length = strlen(s1->s);
        char* temp = (char*)platform_realloc(s1->s, s1Length + 2 + 1);/*2 because 2 quotes, 1 because '\0'*/
        if (temp == NULL)
        {
            LogError("Failure reallocating value.");
...
int STRING_empty(STRING_HANDLE handle)
{
    ...
    {
        STRING* s1 = (STRING*)handle;
        char* temp = (char*)platform_realloc(s1->s, 1);
        if (temp == NULL)
        {
            LogError("Failure reallocating value.");
...
STRING_HANDLE STRING_construct_n(const char* psz, size_t n)
{
    ...
    else
    {
        ...
        else
        {
            ...
            if ((str = (STRING*)platform_malloc(sizeof(STRING))) != NULL)
            {
                if ((str->s = (char*)platform_malloc(len + 1)) != NULL)
                {
...
STRING_HANDLE STRING_from_byte_array(const unsigned char* source, size_t size)
{
    ...
    else
    {
        ...
        {
            /*Codes_SRS_STRING_02_023: [ Otherwise, STRING_from_BUFFER shall build a string that has the same content (byte-by-byte) as source and return a non-NULL handle. ]*/
            result->s = (char*)platform_malloc(size + 1);
            if (result->s == NULL)
            {
                /*Codes_SRS_STRING_02_024: [ If building the string fails, then STRING_from_BUFFER shall fail and return NULL. ]*/
                LogError("oom - unable to malloc");
