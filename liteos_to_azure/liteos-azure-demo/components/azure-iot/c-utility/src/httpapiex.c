...
HTTPAPIEX_HANDLE HTTPAPIEX_Create(const char* hostName)
{
    ...
    else
    {
        ...
        HTTPAPIEX_HANDLE_DATA* handleData = (HTTPAPIEX_HANDLE_DATA*)platform_malloc(sizeof(HTTPAPIEX_HANDLE_DATA));
        if (handleData == NULL)
        {
...
void HTTPAPIEX_Destroy(HTTPAPIEX_HANDLE handle)
{
    if (handle != NULL)
    {
        ...
        for (i = 0; i < vectorSize; i++)
        {
            ...
            platform_free((void*)savedOption->optionName);
            platform_free((void*)savedOption->value);
        }
...
static int createOrUpdateOption(HTTPAPIEX_HANDLE_DATA* handleData, const char* optionName, const void* value)
{
    ...
    if (whereIsIt != NULL)
    {
        platform_free((void*)(whereIsIt->value));
    ...
    else
    {
        ...
        else
        {
            ...
            if (VECTOR_push_back(handleData->savedOptions, &newOption, 1) != 0)
            {
                ...
                platform_free((void*)newOption.optionName);
                platform_free((void*)value);
                result = __FAILURE__;
            }
            else
