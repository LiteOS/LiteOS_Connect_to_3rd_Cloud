...
HTTP_HEADERS_HANDLE HTTPHeaders_Alloc(void)
{
    ...
    result = (HTTP_HEADERS_HANDLE_DATA*)platform_malloc(sizeof(HTTP_HEADERS_HANDLE_DATA));
    ...
    else
    {
        ...
        else
        {
            ...
            if (!replace && (existingValue != NULL))
            {
                ...
                char* newValue = (char*)platform_malloc(sizeof(char) * (existingValueLen + /*COMMA_AND_SPACE_LENGTH*/ 2 + valueLen + /*EOL*/ 1));
                if (newValue == NULL)
                {
    ...
    else
    {
        ...
        else
        {
            ...
            else
            {
                ...
                *destination = (char*)platform_malloc(sizeof(char) * (keyLen + /*COLON_AND_SPACE_LENGTH*/ 2 + valueLen + /*EOL*/ 1));
                if (*destination == NULL)
                {
                    /*Codes_SRS_HTTP_HEADERS_99_034:[ The function shall return HTTP_HEADERS_ERROR when an internal error occurs]*/
                    result = HTTP_HEADERS_ERROR;
