...
static void DestroyDiagnosticPropertyData(IOTHUB_MESSAGE_DIAGNOSTIC_PROPERTY_DATA_HANDLE diagnosticHandle)
{
    ...
    {
        platform_free(diagnosticHandle->diagnosticId);
        platform_free(diagnosticHandle->diagnosticCreationTimeUtc);
    }
    platform_free(diagnosticHandle);
}
...
static void DestroyMessageData(IOTHUB_MESSAGE_HANDLE_DATA* handleData)
{
    ...
    platform_free(handleData->messageId);
    handleData->messageId = NULL;
    platform_free(handleData->correlationId);
    handleData->correlationId = NULL;
    platform_free(handleData->userDefinedContentType);
    platform_free(handleData->contentEncoding);
...
static IOTHUB_MESSAGE_DIAGNOSTIC_PROPERTY_DATA_HANDLE CloneDiagnosticPropertyData(const IOTHUB_MESSAGE_DIAGNOSTIC_PROPERTY_DATA* source)
{
    ...
    {
        result = (IOTHUB_MESSAGE_DIAGNOSTIC_PROPERTY_DATA_HANDLE)platform_malloc(sizeof(IOTHUB_MESSAGE_DIAGNOSTIC_PROPERTY_DATA));
        if (result == NULL)
        {
        ...
        else
        {
            ...
            else if (source->diagnosticId != NULL && mallocAndStrcpy_s(&result->diagnosticId, source->diagnosticId) != 0)
            {
                ...
                platform_free(result->diagnosticCreationTimeUtc);
                platform_free(result);
                result = NULL;
            }
        }
...
IOTHUB_MESSAGE_HANDLE IoTHubMessage_CreateFromByteArray(const unsigned char* byteArray, size_t size)
{
    ...
    {
        result = (IOTHUB_MESSAGE_HANDLE_DATA*)platform_malloc(sizeof(IOTHUB_MESSAGE_HANDLE_DATA));
        if (result == NULL)
        {
