...
{
    IOTHUB_MESSAGE_DIAGNOSTIC_PROPERTY_DATA* result = (IOTHUB_MESSAGE_DIAGNOSTIC_PROPERTY_DATA*)platform_malloc(sizeof(IOTHUB_MESSAGE_DIAGNOSTIC_PROPERTY_DATA));
    if (result == NULL)
    {
    ...
    {
        char* diagId = (char*)platform_malloc(9);
        if (diagId == NULL)
        {
        ...
        else
        {
            ...
            timeBuffer = (char*)platform_malloc(TIME_STRING_BUFFER_LEN);
            if (timeBuffer == NULL)
            {
                LogError("malloc for timeBuffer failed");
                platform_free(result->diagnosticId);
                platform_free(result);
                result = NULL;
            }
...
int IoTHubClient_Diagnostic_AddIfNecessary(IOTHUB_DIAGNOSTIC_SETTING_DATA* diagSetting, IOTHUB_MESSAGE_HANDLE messageHandle)
{
    ...
    else if (should_add_diagnostic_info(diagSetting))
    {
        ...
        else
        {
            ...
            platform_free(diagnosticData->diagnosticCreationTimeUtc);
            platform_free(diagnosticData->diagnosticId);
            platform_free(diagnosticData);
            diagnosticData = NULL;
        }
    }
    else
