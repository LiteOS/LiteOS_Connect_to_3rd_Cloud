...
static void freeUploadToBlobThreadInfo(UPLOADTOBLOB_THREAD_INFO* threadInfo)
{
    ...
    platform_free(threadInfo->uploadBlobSavedData.source);
    platform_free(threadInfo->destinationFileName);
    platform_free(threadInfo);
}
...
static void iothub_ll_device_twin_callback(DEVICE_TWIN_UPDATE_STATE update_state, const unsigned char* payLoad, size_t size, void* userContextCallback)
{
    ...
    if (queue_context != NULL)
    {
        ...
        {
            queue_cb_info.iothub_callback.dev_twin_cb_info.payLoad = (unsigned char*)platform_malloc(size);
        ...
        if (push_to_vector == 0)
        {
            if (VECTOR_push_back(queue_context->iotHubClientHandle->saved_user_callback_list, &queue_cb_info, 1) != 0)
            {
                if (queue_cb_info.iothub_callback.dev_twin_cb_info.payLoad != NULL)
                {
                    platform_free(queue_cb_info.iothub_callback.dev_twin_cb_info.payLoad);
                }
        ...
        else
        {
            switch (queued_cb->type)
            {
                case CALLBACK_TYPE_DEVICE_TWIN:
                {
                    ...
                    {
                        platform_free(queued_cb->iothub_callback.dev_twin_cb_info.payLoad);
                    }
                    break;
                }
...
static IOTHUB_CLIENT_INSTANCE* create_iothub_instance(const IOTHUB_CLIENT_CONFIG* config, TRANSPORT_HANDLE transportHandle, const char* connectionString, IOTHUB_CLIENT_TRANSPORT_PROVIDER protocol, const char* iothub_uri, const char* device_id)
{
    ...
    IOTHUB_CLIENT_INSTANCE* result = (IOTHUB_CLIENT_INSTANCE*)platform_malloc(sizeof(IOTHUB_CLIENT_INSTANCE));
...
void IoTHubClient_Destroy(IOTHUB_CLIENT_HANDLE iotHubClientHandle)
{
    ...
    if (iotHubClientHandle != NULL)
    {
        ...
        {
            platform_free(iotHubClientInstance->devicetwin_user_context);
        }
        if (iotHubClientInstance->connection_status_user_context != NULL)
        {
            platform_free(iotHubClientInstance->connection_status_user_context);
        }
        if (iotHubClientInstance->message_user_context != NULL)
        {
            platform_free(iotHubClientInstance->message_user_context);
        }
        if (iotHubClientInstance->method_user_context != NULL)
        {
            platform_free(iotHubClientInstance->method_user_context);
        }
        platform_free(iotHubClientInstance);
    }
}
        ...
        else
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    IOTHUB_QUEUE_CONTEXT* queue_context = (IOTHUB_QUEUE_CONTEXT*)platform_malloc(sizeof(IOTHUB_QUEUE_CONTEXT));
                    if (queue_context == NULL)
                    {
                        result = IOTHUB_CLIENT_ERROR;
    ...
    else
    {
        ...
        else
        {
            ...
            else
            {
                ...
                {
                    iotHubClientInstance->message_user_context = (IOTHUB_QUEUE_CONTEXT*)platform_malloc(sizeof(IOTHUB_QUEUE_CONTEXT));
                    if (iotHubClientInstance->message_user_context == NULL)
                    {
                        result = IOTHUB_CLIENT_ERROR;
                        LogError("Failed allocating QUEUE_CONTEXT");
        ...
        else
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    iotHubClientInstance->connection_status_user_context = (IOTHUB_QUEUE_CONTEXT*)platform_malloc(sizeof(IOTHUB_QUEUE_CONTEXT));
                    if (iotHubClientInstance->connection_status_user_context == NULL)
                    {
                        result = IOTHUB_CLIENT_ERROR;
                        LogError("Failed allocating QUEUE_CONTEXT");
        ...
        else
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    iotHubClientInstance->devicetwin_user_context = (IOTHUB_QUEUE_CONTEXT*)platform_malloc(sizeof(IOTHUB_QUEUE_CONTEXT));
                    if (iotHubClientInstance->devicetwin_user_context == NULL)
                    {
                        result = IOTHUB_CLIENT_ERROR;
                        LogError("Failed allocating QUEUE_CONTEXT");
    ...
    else
    {
        ...
        else
        {
            ...
            else
            {
                ...
                {
                    iotHubClientInstance->method_user_context = (IOTHUB_QUEUE_CONTEXT*)platform_malloc(sizeof(IOTHUB_QUEUE_CONTEXT));
                    if (iotHubClientInstance->method_user_context == NULL)
                    {
                        result = IOTHUB_CLIENT_ERROR;
                        LogError("Failed allocating QUEUE_CONTEXT");
...
{
    UPLOADTOBLOB_THREAD_INFO* threadInfo = (UPLOADTOBLOB_THREAD_INFO*)platform_malloc(sizeof(UPLOADTOBLOB_THREAD_INFO));
    if (threadInfo == NULL)
    {
...
static IOTHUB_CLIENT_RESULT initializeUploadToBlobData(UPLOADTOBLOB_THREAD_INFO* threadInfo, const unsigned char* source, size_t size, IOTHUB_CLIENT_FILE_UPLOAD_CALLBACK iotHubClientFileUploadCallback)
{
    ...
    if (size != 0)
    {
        if ((threadInfo->uploadBlobSavedData.source = (unsigned char*)platform_malloc(size)) == NULL)
        {
