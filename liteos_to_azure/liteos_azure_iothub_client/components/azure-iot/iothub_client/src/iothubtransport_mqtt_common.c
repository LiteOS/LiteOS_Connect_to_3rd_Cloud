...
{
    int result;
    MQTT_DEVICE_TWIN_ITEM* mqtt_info = (MQTT_DEVICE_TWIN_ITEM*)platform_malloc(sizeof(MQTT_DEVICE_TWIN_ITEM));
    if (mqtt_info == NULL)
    {
                            ...
                            if (isSystemProperty(tokenData))
                            {
                                ...
                                while (iterator != NULL && *iterator != '\0' && result == 0)
                                {
                                    if (*iterator == '=')
                                    {
                                        ...
                                        char* propName = platform_malloc(nameLen + 1);

                                        size_t valLen = tokenLen - (nameLen + 1) + 1;
                                        char* propValue = platform_malloc(valLen + 1);
                                        ...
                                        platform_free(propName);
                                        platform_free(propValue);
    ...
    if (msgHandle != NULL && callbackCtx != NULL)
    {
        ...
        else
        {
            ...
            else if (type == IOTHUB_TYPE_DEVICE_METHODS)
            {
                ...
                {
                    DEVICE_METHOD_INFO* dev_method_info = platform_malloc(sizeof(DEVICE_METHOD_INFO) );
            ...
            else
            {
                ...
                else
                {
                    ...
                    {
                        MESSAGE_CALLBACK_INFO* messageData = (MESSAGE_CALLBACK_INFO*)platform_malloc(sizeof(MESSAGE_CALLBACK_INFO));
                        if (messageData == NULL)
                        {
...
{
    PMQTTTRANSPORT_HANDLE_DATA state = (PMQTTTRANSPORT_HANDLE_DATA)platform_malloc(sizeof(MQTTTRANSPORT_HANDLE_DATA));
    if (state == NULL)
    {
...
void IoTHubTransport_MQTT_Common_Destroy(TRANSPORT_LL_HANDLE handle)
{
    ...
    if (transport_data != NULL)
    {
        ...
        while (!DList_IsListEmpty(&transport_data->ack_waiting_queue))
        {
            ...
            platform_free(mqtt_device_twin);
        }
            ...
            else if (transport_data->currPacketState == PUBLISH_TYPE)
            {
                ...
                while (currentListEntry != transport_data->waitingToSend)
                {
                    ...
                    else
                    {
                        ...
                        MQTT_MESSAGE_DETAILS_LIST* mqttMsgEntry = (MQTT_MESSAGE_DETAILS_LIST*)platform_malloc(sizeof(MQTT_MESSAGE_DETAILS_LIST));
    ...
    else
    {
        ...
        else if (strcmp(OPTION_HTTP_PROXY, option) == 0)
        {
            ...
            else
            {
                ...
                {
                    /* Codes_SRS_IOTHUB_TRANSPORT_MQTT_COMMON_01_004: [ If copying `host_address`, `username` or `password` fails, `IoTHubTransport_MQTT_Common_SetOption` shall fail and return `IOTHUB_CLIENT_ERROR`. ]*/
                    platform_free(copied_proxy_hostname);
                ...
                else if ((proxy_options->password != NULL) && (mallocAndStrcpy_s(&copied_proxy_password, proxy_options->password) != 0))
                {
                    ...
                    {
                        platform_free(copied_proxy_username);
                    }
                    platform_free(copied_proxy_hostname);
