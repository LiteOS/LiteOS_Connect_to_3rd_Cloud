...
{
    int result;
    PENDING_SOCKET_IO* pending_socket_io = (PENDING_SOCKET_IO*)platform_malloc(sizeof(PENDING_SOCKET_IO));
    ...
    {
        pending_socket_io->bytes = (unsigned char*)platform_malloc(size);
        if (pending_socket_io->bytes == NULL)
        {
            platform_free(pending_socket_io);
            result = __FAILURE__;
        }
        else
        {
            ...
            {
                platform_free(pending_socket_io->bytes);
                platform_free(pending_socket_io);
                result = __FAILURE__;
            }
            else
...
CONCRETE_IO_HANDLE socketio_create(void* io_create_parameters)
{
    ...
    {
        result = platform_malloc(sizeof(SOCKET_IO_INSTANCE));
        if (result != NULL)
        {
            ...
            {
                result->hostname = (char*)platform_malloc(strlen(socket_io_config->hostname) + 1);
...
void socketio_destroy(CONCRETE_IO_HANDLE socket_io)
{
    if (socket_io != NULL)
    {
        ...
        platform_free(socket_io_instance->hostname);
        platform_free(socket_io);
    }
}
...
void socketio_dowork(CONCRETE_IO_HANDLE socket_io)
{
    if (socket_io != NULL)
    {
        ...
        if (socket_io_instance->io_state == IO_STATE_OPEN)
        {
            ...
            while (received > 0)
            {
                unsigned char* recv_bytes = platform_malloc(MBED_RECEIVE_BYTES_VALUE);
                if (recv_bytes == NULL)
                {
