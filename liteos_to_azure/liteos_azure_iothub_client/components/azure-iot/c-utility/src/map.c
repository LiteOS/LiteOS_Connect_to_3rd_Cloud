...
MAP_HANDLE Map_Create(MAP_FILTER_CALLBACK mapFilterFunc)
{
    ...
    MAP_HANDLE_DATA* result = (MAP_HANDLE_DATA*)platform_malloc(sizeof(MAP_HANDLE_DATA));
...
void Map_Destroy(MAP_HANDLE handle)
{
    ...
    if (handle != NULL)
    {
        ...
        {
            platform_free(handleData->keys[i]);
            platform_free(handleData->values[i]);
        }
        platform_free(handleData->keys);
        platform_free(handleData->values);
        platform_free(handleData);
    }
}
...
{
    char** result;
    result = (char**)platform_malloc(count *sizeof(char*));
    if (result == NULL)
    {
...
MAP_HANDLE Map_Clone(MAP_HANDLE handle)
{
    ...
    {
        MAP_HANDLE_DATA * handleData = (MAP_HANDLE_DATA *)handle;
        result = (MAP_HANDLE_DATA*)platform_malloc(sizeof(MAP_HANDLE_DATA));
        if (result == NULL)
        {
        ...
        else
        {
            ...
            else
            {
                ...
                else if ((result->values = Map_CloneVector((const char* const*)handleData->values, handleData->count)) == NULL)
                {
                    ...
                    {
                        platform_free(result->keys[i]); 
                    }
                    platform_free(result->keys);
                    platform_free(result);
                    result = NULL;
                }
                else
...
{
    int result;
    char** newKeys = (char**)platform_realloc(handleData->keys, (handleData->count + 1) * sizeof(char*));
    if (newKeys == NULL)
    {
    ...
    else
    {
        ...
        newValues = (char**)platform_realloc(handleData->values, (handleData->count + 1) * sizeof(char*));
        if (newValues == NULL)
        {
            LogError("realloc error");
            ...
            {
                char** undoneKeys = (char**)platform_realloc(handleData->keys, (handleData->count) * sizeof(char*));
                if (undoneKeys == NULL)
                {
...
static void Map_DecreaseStorageKeysValues(MAP_HANDLE_DATA* handleData)
{
    ...
    else
    {
        /*certainly > 1...*/
        char** undoneValues;
        char** undoneKeys = (char**)platform_realloc(handleData->keys, sizeof(char*)* (handleData->count - 1)); 
        if (undoneKeys == NULL)
        {
            LogError("CATASTROPHIC error, unable to undo through realloc to a smaller size");
        }
        ...
        undoneValues = (char**)platform_realloc(handleData->values, sizeof(char*)* (handleData->count - 1));
...
static int insertNewKeyValue(MAP_HANDLE_DATA* handleData, const char* key, const char* value)
{
    ...
    else
    {
        ...
        else
        {
            ...
            {
                platform_free(handleData->keys[handleData->count - 1]);
                Map_DecreaseStorageKeysValues(handleData);
                LogError("unable to mallocAndStrcpy_s");
                result = __FAILURE__;
            }
    ...
    else
    {
        ...
        else
        {
            ...
            else
            {
                ...
                char* newValue = (char*)platform_realloc(handleData->values[index],valueLength  + 1);
                if (newValue == NULL)
                {
                    result = MAP_ERROR;
                    LOG_MAP_ERROR;
...
MAP_RESULT Map_Delete(MAP_HANDLE handle, const char* key)
{
    ...
    else
    {
        ...
        else
        {
            ...
            size_t index = whereIsIt - handleData->keys;
            platform_free(handleData->keys[index]);
            platform_free(handleData->values[index]);
