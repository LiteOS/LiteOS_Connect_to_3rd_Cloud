...
static void on_underlying_io_bytes_received(void* context, const unsigned char* buffer, size_t size)
{
    if (context != NULL)
    {
        ...
        unsigned char* new_socket_io_read_bytes = (unsigned char*)platform_realloc(tls_io_instance->socket_io_read_bytes, tls_io_instance->socket_io_read_byte_count + size);
...
static int on_io_recv(WOLFSSL *ssl, char *buf, int sz, void *context)
{
    ...
    if (context != NULL)
    {
        ...
        if (result > 0)
        {
            ...
            {
                new_socket_io_read_bytes = (unsigned char*)platform_realloc(tls_io_instance->socket_io_read_bytes, tls_io_instance->socket_io_read_byte_count);
                if (new_socket_io_read_bytes != NULL)
                {
                    tls_io_instance->socket_io_read_bytes = new_socket_io_read_bytes;
                }
            ...
            {
                platform_free(tls_io_instance->socket_io_read_bytes);
...
void tlsio_wolfssl_destroy(CONCRETE_IO_HANDLE tls_io)
{
    if (tls_io != NULL)
    {
        ...
        {
            platform_free(tls_io_instance->x509certificate);
            tls_io_instance->x509certificate = NULL;
        }
        if (tls_io_instance->x509privatekey != NULL)
        {
            platform_free(tls_io_instance->x509privatekey);
            tls_io_instance->x509privatekey = NULL;
        }
        destroy_wolfssl_instance(tls_io_instance);
