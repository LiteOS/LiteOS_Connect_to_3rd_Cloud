...
INLINE int blake2b_set_lastnode( blake2b_state *S )
{
  S->f[1] = ~0ULL;
  return 0;
}
...
INLINE int blake2b_set_lastblock( blake2b_state *S )
{
...
INLINE int blake2b_increment_counter( blake2b_state *S, const word64
                                             inc )
{
  S->t[0] += inc;
  S->t[1] += ( S->t[0] < inc );
...
INLINE int blake2b_init0( blake2b_state *S )
{
  int i;
