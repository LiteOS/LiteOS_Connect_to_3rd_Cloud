            ...
            const char *timestamp_str)
{
    ...
    dev = (iotx_device_info_pt)iotx_device_info_get();
    assert(dev);
            ...
            uint16_t *pport)
{
    ...
    int                 iotx_port = 443;
    int                 ret = -1;
    iotx_conn_info_pt   usr = (iotx_conn_info_pt)iotx_conn_info_get();
    int                 ret_code = 0;
...
int iotx_guider_authenticate(void)
{
    ...
    iotx_device_info_pt dev = (iotx_device_info_pt)iotx_device_info_get();
    iotx_conn_info_pt   usr = (iotx_conn_info_pt)iotx_conn_info_get();
    char               *req_str = NULL;
    //httpclient_t       httpc;
    ...
    usr->pub_key = (const char *)iotx_ca_get();
#ifdef MQTT_DIRECT
