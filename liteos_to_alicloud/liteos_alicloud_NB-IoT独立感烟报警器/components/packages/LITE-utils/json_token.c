...
void LITE_json_keys_release(LOS_DL_LIST *keylist)
{
    json_key_t         *pos, *tmp;
    
    //list_for_each_entry_safe(pos, tmp, keylist, json_key_t, list) {
    LOS_DL_LIST_FOR_EACH_ENTRY_SAFE(pos, tmp, keylist, json_key_t, list) {
        if (pos->key) {
            LITE_free(pos->key);
        }
        LOS_ListDelete(&pos->list);
        LITE_free(pos);
    }
}

LOS_DL_LIST *LITE_json_keys_of(char *src, char *prefix)
{
    static              LOS_DL_LIST_HEAD(keylist);
    ...
    json_object_for_each_kv(src, pos, key, klen, val, vlen, vtype) {
        if (key && klen && val && vlen) {
            ...
            LOS_ListTailInsert(&keylist, &entry->list);
