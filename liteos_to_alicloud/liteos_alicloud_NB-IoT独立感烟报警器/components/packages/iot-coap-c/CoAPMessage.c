...
static int CoAPMessageList_add(CoAPContext *context, CoAPMessage *message, int len)
{
    ...
    node = (CoAPSendNode *)coap_malloc(sizeof(CoAPSendNode));
    ...
    if (NULL != node) {
        ...
        if (&context->list.count >= &context->list.maxcount) {
            ...
            LOS_ListTailInsert(&context->list.sendlist, &node->sendlist);
            context->list.count ++;
            return 0;
        }
    } else {
...
static int CoAPAckMessage_handle(CoAPContext *context, CoAPMessage *message)
{
    ...
    LOS_DL_LIST_FOR_EACH_ENTRY(node, &context->list.sendlist, CoAPSendNode, sendlist) {
...
int CoAPMessage_cycle(CoAPContext *context)
{
    ...
    LOS_DL_LIST_FOR_EACH_ENTRY_SAFE(node, next, &context->list.sendlist, CoAPSendNode, sendlist) {
        if (NULL != node) {
            if (node->timeout == 0) {
