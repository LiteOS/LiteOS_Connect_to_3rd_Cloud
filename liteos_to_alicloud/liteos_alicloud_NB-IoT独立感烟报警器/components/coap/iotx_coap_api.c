...
#ifdef REPORT_TYPE_PROP
/* report prop */
int iotx_coap_report_prop(iotx_coap_context_t *p_context, char smoke, char remove, char bat, int period)
{
    int                     ret;
    ...
    iotx_propreport_payload_sdm200(msg, smoke, remove, bat, period);
                               ...
                               unsigned int block_type, unsigned int num, unsigned int more, unsigned int size)
{
    ...
    if (IOTX_MESSAGE_CON > (int8_t)p_message->msg_type || IOTX_MESSAGE_NON < (int8_t)p_message->msg_type
        || IOTX_CONTENT_TYPE_JSON > (int8_t)p_message->content_type
        || IOTX_CONTENT_TYPE_CBOR < (int8_t)p_message->content_type) {
