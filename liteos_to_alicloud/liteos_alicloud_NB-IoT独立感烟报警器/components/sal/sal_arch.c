...
{
    err_t ret = ERR_MEM;
    int stat = LOS_SemCreate(count, sem);
...
uint32_t sal_arch_sem_wait(sal_sem_t *sem, uint32_t timeout)
{
    ...
    if ( timeout != 0UL ) {
        ...
        while ( !(LOS_SemPend(*sem, LOS_WAIT_FOREVER) == 0));
        end_ms = sal_now();
...
err_t sal_mbox_new(sal_mbox_t *mb, int size)
{
    ...
    int stat = LOS_QueueCreate("sal mbox", size * sizeof(void *), mb, 0, sizeof(void *));
...
{
    LOS_QueueWrite(*mb,msg,sizeof(void*),0);
}
...
{
    if (LOS_QueueWrite(*mb,msg,sizeof(void*),0) != 0)
        return ERR_MEM;
    else
        return ERR_OK;
}
...
u32_t sal_arch_mbox_fetch(sal_mbox_t *mb, void **msg, u32_t timeout)
{
    ...
    if( timeout != 0UL ) {
        ...
        if(LOS_QueueRead(*mb,msg,len,timeout) == 0) {
            end_ms = sal_now();
            elapsed_ms = end_ms - begin_ms;
            ret = elapsed_ms;
        } else {
        ...
        while(LOS_QueueRead(*mb,msg,len,LOS_WAIT_FOREVER) != 0);
        end_ms = sal_now();
        elapsed_ms = end_ms - begin_ms;
...
u32_t sal_arch_mbox_tryfetch(sal_mbox_t *mb, void **msg)
{
    ...
    if(LOS_QueueRead(*mb,msg,len,0u) != 0 ) {
...
{
    LOS_MuxPend(sal_arch_mutex, LOS_WAIT_FOREVER);
    return 0;
}
...
{
    LOS_MuxCreate(&sal_arch_mutex);
}

void sal_mutex_arch_free(void)
{
    LOS_MuxDelete(sal_arch_mutex);
}
