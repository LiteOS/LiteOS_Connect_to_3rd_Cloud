...
static void sal_tcp_remove(struct tcp_pcb *pcb)
{
    ...
    LOS_MemFree(OS_SYS_MEM_ADDR, pcb);
...
static void iphex_To_ipstr(uint32_t ip_hex, char *ip)
{
    union {
        uint32_t ip_u32;
        uint8_t ip_u8[4];
...
static int salpcb_new(sal_netconn_t *conn)
{
    ...
    switch (NETCONNTYPE_GROUP(conn->type)) {
        case NETCONN_RAW:
            conn->pcb.raw = LOS_MemAlloc(OS_SYS_MEM_ADDR, sizeof(struct raw_pcb));
            ...
            break;
        case NETCONN_UDP:
            conn->pcb.udp = LOS_MemAlloc(OS_SYS_MEM_ADDR, sizeof(struct udp_pcb));
            ...
            break;
        case NETCONN_TCP:
            conn->pcb.tcp = LOS_MemAlloc(OS_SYS_MEM_ADDR, sizeof(struct tcp_pcb));
            if (NULL == conn->pcb.tcp) {
                return ERR_MEM;
            }
...
static void salnetconn_drain(sal_netconn_t *conn)
{
    ...
    if (sal_mbox_valid(&conn->recvmbox)){
        while(sal_mbox_tryfetch(&conn->recvmbox, (void **)(&mem)) != SAL_MBOX_EMPTY){
            if (mem != NULL){
                if (mem->payload){
                    LOS_MemFree(OS_SYS_MEM_ADDR, mem->payload);
                    mem->payload = NULL;
                }
                LOS_MemFree(OS_SYS_MEM_ADDR, mem);
            }
        }
...
static sal_netconn_t *salnetconn_new(enum netconn_type t)
{
    ...
    conn = (sal_netconn_t *)LOS_MemAlloc(OS_SYS_MEM_ADDR, sizeof(sal_netconn_t));
    if (conn == NULL) {
...
#endif
    LOS_MemFree(OS_SYS_MEM_ADDR, conn);
...
static err_t salnetconn_delete(sal_netconn_t *conn)
{
    ...
    if (NULL != conn->pcb.tcp) {
        ...
        conn->pcb.tcp = NULL;
        LOS_MemFree(OS_SYS_MEM_ADDR, conn->pcb.tcp);
    }
    ...
    do{
        ...
        if ((flags & MSG_PEEK)  == 0){
            if ((NETCONNTYPE_GROUP(pstsock->conn->type) == NETCONN_TCP) && (buflen > copylen)){
                ...
                LOS_MemFree(OS_SYS_MEM_ADDR, buf->payload);
                LOS_MemFree(OS_SYS_MEM_ADDR, buf);
                buf = NULL;
            }
        }
    }while(!done);
               ...
               const struct sockaddr *to, socklen_t tolen)
{
...
#if SAL_PACKET_SEND_MODE_ASYNC
    buf = (sal_outputbuf_t *)LOS_MemAlloc(OS_SYS_MEM_ADDR, sizeof(sal_outputbuf_t));
    if (NULL == buf){
    ...
    buf->payload = LOS_MemAlloc(OS_SYS_MEM_ADDR, size);
    if (NULL == buf->payload){
        LOS_MemFree(OS_SYS_MEM_ADDR, buf);
...
#else
    sal_deal_event(s, NETCONN_EVT_SENDMINUS);
    iphex_To_ipstr(*(uint32_t *)&pstsalsock->conn->pcb.udp->remote_ip.u_addr.ip4, (char *)ip_str);

    //if (sal_module_send(s, (uint8_t *)data, size, NULL, -1, pstsalsock->conn->send_timeout)){
    if (sal_module_send(s, (uint8_t *)data, size, (char *)ip_str, \
                            pstsalsock->conn->pcb.udp->remote_port, pstsalsock->conn->send_timeout)){
...
static int salnetconn_packet_input(sal_netconn_t *conn, void *data, size_t len, char remote_ip[16], uint16_t remote_port)
{
    ...
    buf = (sal_netbuf_t *)LOS_MemAlloc(OS_SYS_MEM_ADDR, sizeof(sal_netbuf_t));
    if (NULL == buf){
        SAL_ERROR("memory is not enough, malloc size %d fail\n", sizeof(sal_netbuf_t));
        return -1;
    }
    ...
    buf->payload = LOS_MemAlloc(OS_SYS_MEM_ADDR, len);
    if (NULL == buf->payload){
        LOS_MemFree(OS_SYS_MEM_ADDR, buf);
        SAL_ERROR("memory is no enough, malloc size %d fail\n", len);
        return -1;
    ...
    while(1){
        for (fd = 0; fd < MEMP_NUM_NETCONN; fd++){
            ...
            if (NULL == pstsalsock || NULL == pstsalsock->conn || !sal_mbox_valid(&pstsalsock->conn->sendmbox)){
                LOS_TaskDelay(SAL_ASYNC_TASK_SLEEP_TIME_PER_SOCKET);
                continue;
            }
            ...
            if (sal_arch_mbox_fetch(&pstsalsock->conn->sendmbox, (void **)&outputmem, SAL_ASYNC_TASK_SLEEP_TIME_PER_SOCKET) != SAL_ARCH_TIMEOUT){
                ...
                LOS_MemFree(OS_SYS_MEM_ADDR, outputmem->payload);
                LOS_MemFree(OS_SYS_MEM_ADDR, outputmem);
                outputmem = NULL;
            }
        }
    }
...
int sal_init(void)
{
    static unsigned char sal_init_done = 0;
#if SAL_PACKET_SEND_MODE_ASYNC
    aos_task_t  task;
#endif
    if (sal_init_done) {
                    ...
                    const struct addrinfo *hints, struct addrinfo **res)
{
    ...
    *res = NULL;
    ...
    ai = (struct addrinfo *)LOS_MemAlloc(OS_SYS_MEM_ADDR, total_size);
    if (ai == NULL) {
        return EAI_MEMORY;
    }
...
{
    if (ai != NULL) {
        LOS_MemFree(OS_SYS_MEM_ADDR, ai);
    }
}
