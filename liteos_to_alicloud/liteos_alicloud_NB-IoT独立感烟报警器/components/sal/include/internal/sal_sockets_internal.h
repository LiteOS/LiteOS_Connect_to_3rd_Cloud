...
#ifdef SAL_USE_DEBUG
#define SAL_DEBUG(format, ...)  sal_log_print(SAL_TAG, format, ##__VA_ARGS__)
#else
#define SAL_DEBUG(format, ...)
#endif

#define SAL_ERROR(format, ...)  sal_log_print("\r\n[SAL_ERROR]", format, ##__VA_ARGS__)
#define SAL_ASSERT(msg, assertion) //do { if (!(assertion)) { \
        //sal_log_print(SAL_TAG, msg);} \
    //} while (0)
