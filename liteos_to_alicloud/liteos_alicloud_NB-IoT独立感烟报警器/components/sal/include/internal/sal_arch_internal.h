...
typedef uint32_t sal_sem_t;

#define sal_sem_valid(sem)           (sem != NULL) 
#define sal_sem_set_invalid(sem)     do { if(sem != NULL) { (sem) = NULL; }}while(0)

typedef uint32_t sal_prot_t;

typedef uint32_t sal_mutex_t;
#define sal_mutex_valid(mutex)       (mutex != NULL)
#define sal_mutex_set_invalid(mutex) do { if(mutex != NULL) { (mutex) = NULL; }}while(0)

typedef uint32_t sal_mbox_t;
#define sal_mbox_valid(mbox)       (mbox != NULL)
#define sal_mbox_set_invalid(mbox) do { if(mbox != NULL) { (mbox) = NULL; }}while(0)
