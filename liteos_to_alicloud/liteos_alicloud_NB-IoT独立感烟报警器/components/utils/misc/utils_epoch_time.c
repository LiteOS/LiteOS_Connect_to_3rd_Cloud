...
static UINT64 _get_timestamp_from_ntp(const char *host)
{
    long fd;  /* socket */
    int ret = -1;
    UINT64 retval = 0;
    ...
    retval = ((UINT64)tv.tv_sec) * 1000 + tv.tv_usec / 1000;
...
UINT64 utils_get_epoch_time_from_ntp(char copy[], int len)
{
    char ntp_server[20] = {0};
    int ntp_server_index = 1;
    static UINT64 time_in_ms = 0;
    ...
    if (last_tick == 0) {
        ...
        time_in_ms = time_in_ms + (LOS_TickCountGet() - last_tick);
        last_tick = LOS_TickCountGet();
    }
    
    snprintf(copy, len, "%llu", time_in_ms);
    printf("utils_get_epoch_time_from_ntp : %llu / %llu\r\n", time_in_ms, last_tick);
