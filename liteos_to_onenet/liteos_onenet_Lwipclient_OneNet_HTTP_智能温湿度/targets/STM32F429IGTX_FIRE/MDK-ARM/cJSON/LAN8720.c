...
__align(4) ETH_DMADESCTypeDef *DMARxDscrTab1;	//以太网DMA接收描述符数据结构体指针
__align(4) ETH_DMADESCTypeDef *DMATxDscrTab1;	//以太网DMA发送描述符数据结构体指针
__align(4) uint8_t *Rx_Buff1; 					//以太网底层驱动接收buffers指针
__align(4) uint8_t *Tx_Buff1; 					//以太网底层驱动发送buffers指针
...
{
    DMARxDscrTab1=mymalloc(SRAMIN,ETH_RXBUFNB*sizeof(ETH_DMADESCTypeDef));//申请内存
    DMATxDscrTab1=mymalloc(SRAMIN,ETH_TXBUFNB*sizeof(ETH_DMADESCTypeDef));//申请内存
    Rx_Buff1=mymalloc(SRAMIN,ETH_RX_BUF_SIZE*ETH_RXBUFNB);	//申请内存
    Tx_Buff1=mymalloc(SRAMIN,ETH_TX_BUF_SIZE*ETH_TXBUFNB);	//申请内存
    if(!DMARxDscrTab1||!DMATxDscrTab1||!Rx_Buff||!Tx_Buff)
    {
        ETH_Mem_Free();
        return 1;	//申请失败
    }
