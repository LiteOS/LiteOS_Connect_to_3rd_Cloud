...
static int MqttSample_CmdSavedata(struct MqttSampleContext *ctx);
...
static const struct Command commands[] = {
    {"0", MqttSample_CmdConnect, "Establish TCP & MQTT connection."},
    {"1", MqttSample_CmdPing, "Send ping packet."},
    {"2", MqttSample_CmdSavedata,"Save data points(-q Qos -t datatype).\r\n\t  "
		                         "usage: 2 -q 0/1 -t 1/7\r\n\t    "
		                         "-q 0:QoS0 -1:Qos1 default(1)\r\n\t    "
		                         "-t 1:full json 7:float default(1)"},
    {"3", MqttSample_CmdPublish,"Publish to topic.\r\n\t  "
		                        "usage: 3 topic payload packtid"},
    {"4", MqttSample_CmdSubscribe, "Subscribe one topic.\r\n\t  "
		                           "usage: 5 topic"},
    {"5", MqttSample_CmdUnsubscribe, "Unsubscribe one topic.\r\n\t  "
		                             "usage: 5 topic"},
	{"6", MqttSample_RespCmdPublish,"reponse cmd to server (-q Qos0/Qos1)"},
    {"7", MqttSample_CmdDisconnect, "Close the connection."},
    {"8", MqttSample_CmdExit, "Exit the sample."},
    {"9", MqttSample_CmdHelp, "Print the usage of the commands."}
};


char** str_split(char* a_str, const char a_delim, int* sptr_len)
{
    char** result    = 0;
    size_t count     = 0;
    char* tmp        = a_str;
...
//-------------------------------- Commands ------------------------------------------------------
static int MqttSample_CmdConnect(struct MqttSampleContext *ctx, char *prjid, char *auth_info, int keep_alive, int clean_session)
{
    int err, flags;
...
//    const char prjid[] = "70901"; //project_id
//    const char auth_info[] = "0001"; //authoriz info
//    int keep_alive = 1200;
//    int clean_session = 0;
//    ctx->devid = "3264518";  //device_id

	printf("product id: %s\r\nsn: %s\r\ndeviceid: %s\r\nkeepalive: %d\r\ncleansession: %d\r\nQoS: %d\r\n", 
		prjid, auth_info, ctx->devid, keep_alive, clean_session, MQTT_QOS_LEVEL0);
...
#if 0
static int MqttSample_CmdSavedata0(struct MqttSampleContext *ctx)
{
    int err;
...
inline int MqttSample_CmdSavedata11(struct MqttSampleContext *ctx){
    int err;
    struct MqttExtent *ext;
	uint16_t pkt_id = 1; 

    //char t_payload[] = {'\x07','\x00','\x01','\x00','\x01','\x41','\x42','\x43','\x44'};
    char t_json[]="{\"datastreams\":[{\"id\":\"a\",\"datapoints\":[{\"value\":1.3}]},{\"id\":\"b\",\"datapoints\":[{\"value\":2}]}]}";
    ...
    err = Mqtt_PackPublishPkt(ctx->mqttbuf, pkt_id, "$dp", \
		t_payload, payload_len, \
		MQTT_QOS_LEVEL1, 0, 1);
...
inline int MqttSample_CmdSavedata01(struct MqttSampleContext *ctx){
...
inline int MqttSample_CmdSavedata07(struct MqttSampleContext *ctx){
    int err;
    struct MqttExtent *ext;
...
inline int MqttSample_CmdSavedata17(struct MqttSampleContext *ctx){
    /*
    int err = 0;
    int64_t ts;
...
static int MqttSample_CmdSavedata(struct MqttSampleContext *ctx)
{
    ...
    */
    ...
    if(0==Qos){
        if(1==type){
            MqttSample_CmdSavedata01(ctx);
        }else if(7==type){
            MqttSample_CmdSavedata07(ctx);
        }
    }else if(1==Qos){
        if(1==type){
            MqttSample_CmdSavedata11(ctx);
        }else if(7==type){
            MqttSample_CmdSavedata17(ctx);
        }
    }
...
static int MqttSample_CmdSubscribe(struct MqttSampleContext *ctx)
{
    ...
    err = Mqtt_PackSubscribePkt(ctx->mqttbuf, 11, MQTT_QOS_LEVEL1, (const char **)topics+1, topics_len-1);
    if(err != MQTTERR_NOERROR) {
...
static int MqttSample_CmdUnsubscribe(struct MqttSampleContext *ctx)
{
    ...
    err = Mqtt_PackUnsubscribePkt(ctx->mqttbuf, 11, (const char **)topics+1, topics_len-1);
    if(err != MQTTERR_NOERROR) {
...
static int MqttSample_HandleStdin(struct MqttSampleContext *ctx, uint32_t events)
{
	...
	printf("before MqttBuffer_Reset\r\n");
 ...
 * */
    ...
    smpctx->host = MQTT_HOST;
    smpctx->port = MQTT_PORT;
