...
void tcp_echoclient_connect(void)
{
  ...
  if (echoclient_pcb != NULL)
  {
    ...
    if(ERR_OK!=tcp_connect(echoclient_pcb,&DestIPaddr,DEST_PORT,tcp_echoclient_connected))
		{
			printf("connect err.\r\n");
			ConnectFlag=0;
		}
		  ...
		  /* allocate structure es to maintain tcp connection informations */
			...
			if(ConnectFlag==1)
			{
				...
				es->pcb = echoclient_pcb;
				 /* allocate pbuf */
				es->p_tx = pbuf_alloc(PBUF_TRANSPORT, CollectDataLen , PBUF_POOL);
					 
				if (es->p_tx)
				{       
					/* copy data to pbuf */
					pbuf_take(es->p_tx,CollectDataBuf, CollectDataLen);
    ...
    if (es != NULL)
    {
      ...
      {       
        ...
        //tcp_echoclient_send(tpcb,es);
				...
				uwRet = LOS_SwtmrCreate(3000,LOS_SWTMR_MODE_PERIOD,UploadData,&tid,1);
				if(LOS_OK != uwRet)
				{
